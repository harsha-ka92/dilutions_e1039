...
int Fun4Sim(const int nevent = 10)
{
 ...
 //! Particle generator flag.  Only one of these must be true.
  ...
  if(SQ_vtx_gen) { // cf. SQPrimaryVertexGen
    ...
    //rc->set_DoubleFlag("VTX_GEN_Z_STOP" , -180.0); // For "Manual"
  }
  rc->Print();
  ...
  if(gen_e906dim){
    ...
    if(drellyan_gen){
      ...
      e906legacy->set_massRange(6.0, 7.0);
  ...
  // setup Ti 15-3 beam windows (defined in G4_Beamline.C)
  //SetupTiWindow(g4Reco);
  ...
  //
  //chmaber realization to introduce finite chamber resolution of 400 um.
  //
